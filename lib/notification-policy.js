...
export function shouldSendSMS(eventType, hasPhoneNumber = false) {
  ...
  const smsEnabled = readBoolean(process.env.ENABLE_SMS_NOTIFICATIONS, false);
  const criticalOnly = readBoolean(process.env.SMS_CRITICAL_ONLY, true);

  if (!smsEnabled) return false;
  if (!criticalOnly) return true;

  return eventType === "budget-alert" || eventType === "critical-alert";
}