        ...
        if (shouldSendEmail("monthly-report")) {
          await sendEmailWithRetry({
            ...
            templateParams: {
              name: user.name,
              userName: user.name,
              alert_title: `Monthly Financial Report - ${monthName}`,
              alert_message: `Here is your monthly financial summary for ${monthName}.`,
              amount: stats.totalExpenses,
              category: monthName,
              description: `Income: ${stats.totalIncome}, Net: ${stats.totalIncome - stats.totalExpenses}`,
              advice1: insights?.[0] || "",
              advice2: insights?.[1] || "",
              advice3: insights?.[2] || ""
            },
          ...
          if (shouldSendEmail("budget-alert")) {
            await sendEmailWithRetry({
              ...
              templateParams: {
                ...
                alert_title: anomalousTransactions.length > 0 ?
                `Unusual Spending Detected - ${account.name}` :
                `Budget Alert - ${account.name}`,
                ...
                description: `Budget amount: ${budgetAmount}`
              },
              react: EmailTemplate({
                userName: budget.user.name,
                type: "budget-alert",
        ...
        if (shouldSendEmail("budget-coach")) {
          await sendEmailWithRetry({
            ...
            templateParams: {
              name: user.name,
              userName: user.name,
              alert_title: "Your Weekly Budget Digest - Gullak",
              alert_message: "Your weekly budget digest is ready with actionable tips.",
              amount: coachData.stats.totalExpenses,
              category: "Weekly Budget Digest",
              description: `Remaining monthly budget: ${coachData.stats.remainingBudget}`,
              advice1: advice?.[0] || "",
              advice2: advice?.[1] || "",
              advice3: advice?.[2] || ""
            },
