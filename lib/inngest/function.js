...
function buildSpecificMonthlyInsights(stats, month, t) {
  ...
  const localizedTopCategory = translate(`categories.${topCategory}`, {}, topCategory);
  ...
  return [
    translate(
      "notifications.monthlyInsightCashflow",
      {
        month,
        totalExpenses: totalExpenses.toFixed(0),
        totalIncome: totalIncome.toFixed(0),
        savingsRateTarget: Math.max(10, Math.round(savingsRate)).toFixed(0)
      },
    ...
    translate(
      "notifications.monthlyInsightTopCategory",
      {
        topCategory: localizedTopCategory,
        topCategoryAmount: Number(topCategoryAmount).toFixed(0),
        topCategoryShare: topCategoryShare.toFixed(1),
        topCategoryWeeklyCap: Math.max(250, Math.round(Number(topCategoryAmount) / 4))
      },
      `${localizedTopCategory} is your top spend at ₹${Number(topCategoryAmount).toFixed(0)} (${topCategoryShare.toFixed(1)}% of expenses). Set a weekly cap of ₹${Math.max(250, Math.round(Number(topCategoryAmount) / 4))} for this category.`
    ),
    translate(
      "notifications.monthlyInsightWeeklyCeiling",
      { weeklyExpenseCap },
...
async function generateFinancialInsights(stats, month, locale, t) {
  const fallbackInsights = buildSpecificMonthlyInsights(stats, month, t);
  const promptLanguage = getLocalePromptName(locale);
  ...
  const prompt = `
    ...
    Write all insights only in ${promptLanguage} (locale code: ${safeLocale}).
  ...
  join(", ")}
    ...
    Format the response as a JSON array of strings in the target locale language, like this:
  ...
  async ({ step }) => {
    ...
    for (const user of users) {
      await step.run(`generate-report-${user.id}`, async () => {
        ...
        const insights = await generateFinancialInsights(stats, monthName, user.locale, t);
  ...
  async ({ step }) => {
    ...
    for (const user of users) {
      await step.run(`generate-coach-${user.id}`, async () => {
        ...
        const t = getTranslator(user.locale);
        const promptLanguage = getLocalePromptName(user.locale);
        const safeLocale = user.locale || "en";

        const prompt = `
          Analyze this weekly financial data and provide 3 concise, friendly, and actionable money-saving tips for the upcoming week.
          Write all tips only in ${promptLanguage} (locale code: ${safeLocale}).
          Do not mix in English unless locale code is en.
          ...
          Format as JSON array of strings in the target locale language: ["tip 1", "tip 2", "tip 3"]
        `;
        ...
        if (shouldUseScheduledAI()) {
          try {
            ...
            advice = [
            t("notifications.coachAdviceSubscriptions", {}, "Check your recurring subscriptions."),
            t("notifications.coachAdviceCook", {}, "Try cooking at home more often."),
            t("notifications.coachAdviceHabits", {}, "Review your daily spending habits.")];
          ...
          advice = [
          t("notifications.coachAdviceCap", { discretionaryCap }, `For the next 7 days, keep discretionary spending under ₹${discretionaryCap}.`),
          t("notifications.coachAdviceMove", { savingsMove }, `Move ₹${savingsMove} to a savings bucket before mid-week to reduce overspending pressure.`),
          t("notifications.coachAdviceReview", {}, "Review top 3 expenses every Sunday and cut the largest one by at least 10% next week.")];
        }
