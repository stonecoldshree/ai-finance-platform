  ...
  async ({ step }) => {
    ...
    for (const budget of budgets) {
      ...
      await step.run(`check-budget-${budget.id}`, async () => {
        ...
        // Calculate total expenses for the budget's specific account
        ...
        // ── Budget threshold check (rule-based) ──
        const budgetCheck = checkBudgetAnomaly(totalExpenses, Number(budgetAmount));

        // ── Z-score anomaly detection on recent transactions ──
        const recentTransactions = await db.transaction.findMany({
          ...
          orderBy: { date: "desc" },
          take: 100, // last 100 expense transactions as history
        });
        ...
        if (amounts.length >= 5) {
          // Check the most recent 5 transactions for anomalies
          const history = amounts.slice(5); // older transactions as baseline
          const recent = amounts.slice(0, 5); // newest transactions to check
        ...
        // ── Send alert if budget exceeded OR anomalous spending detected ──
        const shouldAlert =
        ...
        if (
          ...
          // Include anomaly info if detected
