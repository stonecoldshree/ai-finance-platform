    ...
    if (existingUserByEmail) {
      if (existingUserByEmail.clerkUserId === user.id) {
        const updatedUser = await db.user.update({
          ...
          data: { name, imageUrl: user.imageUrl, phoneNumber }
        });
        return updatedUser;
      }

      // If this email exists under a different Clerk ID, treat it as stale data
      // and hard-reset it so a newly created Clerk user does not inherit history.
